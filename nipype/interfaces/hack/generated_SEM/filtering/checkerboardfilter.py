...
class CheckerBoardFilterInputSpec(CommandLineInputSpec):
    checkerPattern = InputMultiPath(traits.Int, argstr="--checkerPattern %s", desc="The pattern of input 1 and input 2 in the output image. The user can specify the number of checkers in each dimension. A checkerPattern of 2,2,1 means that images will alternate in every other checker in the first two dimensions. The same pattern will be used in the 3rd dimension.", sep=",")
    inputVolume1 = File(argstr="%s", desc="First Input volume", position=-3, exists=True)
    inputVolume2 = File(argstr="%s", desc="Second Input volume", position=-2, exists=True)
