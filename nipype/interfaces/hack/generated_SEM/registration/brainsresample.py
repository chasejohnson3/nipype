...
class BRAINSResampleInputSpec(CommandLineInputSpec):
    ...
    referenceVolume = File(argstr="--referenceVolume %s", desc="Reference image used only to define the output space. If not specified, the warping is done in the same space as the image to warp.", exists=True)
    ...
    deformationVolume = File(argstr="--deformationVolume %s", desc="Displacement Field to be used to warp the image (ITKv3 or earlier)", exists=True)
    warpTransform = File(argstr="--warpTransform %s", desc="Filename for the BRAINSFit transform (ITKv3 or earlier) or composite transform file (ITKv4)", exists=True)
    ...
    inverseTransform = traits.Bool(argstr="--inverseTransform ", desc="True/False is to compute inverse of given transformation. Default is false")
    defaultValue = traits.Float(argstr="--defaultValue %f", desc="Default voxel value")
    gridSpacing = InputMultiPath(traits.Int, argstr="--gridSpacing %s", desc="Add warped grid to output image to help show the deformation that occured with specified spacing.   A spacing of 0 in a dimension indicates that grid lines should be rendered to fall exactly (i.e. do not allow displacements off that plane).  This is useful for makeing a 2D image of grid lines from the 3D space", sep=",")
